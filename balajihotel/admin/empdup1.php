...
<body>
    <div class="row">
        ...
        <hr>
            ...
            <form class="booking" action="empdup2.php" method="post">
                <div class="panel-body">
                    <div class="form-group">
                        <label> Designation Type: </label> <br>
                        <select class="form-control" name="empd" required>
                            <option selected disabled value=""> Select Designation Type </option>
                            <?php
                            $query  = "SELECT * FROM empdgon";
                            $result = mysqli_query($con, $query);
                            if (mysqli_num_rows($result) > 0) {
                                while ($room_type = mysqli_fetch_assoc($result)) {
                                    echo '<option value="' . $room_type['edid'] . '">' . $room_type['edname'] . '</option>';
                                }
                            }
                        ?>
                        </select><br>
