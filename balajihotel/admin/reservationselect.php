...
<body>
    ...
    <div class="myss" style="overflow-x:auto;">
        ...
        <table id="myTable">
            ...
            $sql = "SELECT customer.cid,customer.fname,customer.lname,customer.ph_no,customer.email,bookings.rno,room_type.roomty,bookings.bdate, 
            bookings.ckindate,bookings.ckoutdate,bookings.pkprice,bookings.nofday,bookings.tprice, customer.invnum,
            bookings.bkstatus FROM customer JOIN bookings JOIN room_type ON (customer.cid=bookings.cid) AND (bookings.rty_id=room_type.rty_id)";   //show all data 
            $rs = mysqli_query($con, $sql); //query run


            while ($row = mysqli_fetch_assoc($rs)) {
                $frd = date('d-m-Y', strtotime($fdt));  // get chk-in and chk-out DATE with  convert dd mm yy 
                $tod = date('d-m-Y', strtotime($row['bdate']));
